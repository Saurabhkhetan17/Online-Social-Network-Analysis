...
def read_screen_names(filename):
    ...
    """
    linesusername = open(filename).read().split("\n")
    linesusername = [x.strip() for x in linesusername]
    return linesusername
...
def get_users(twitter, screen_names):
    ...
    request = robust_request(twitter,resource,params,max_tries=5)
    return request
...
def get_friends(twitter, screen_name):
    ...
    retrieved_ids = sorted([x for x in request])
...
def add_all_friends(twitter, users):
    ...
    for i, item in enumerate(name_user['screen_name'] for name_user in users):
        users[i]['friends'] = get_friends(twitter, item)
...
def print_num_friends(users):
    ...
    for i, item in enumerate([name_user['screen_name'] for name_user in users]):
            print(item, len(users[i]['friends']))
...
def count_friends(users):
    ...
    counter = Counter()
    for i, item in enumerate([name_user['screen_name'] for name_user in users]):
        counter.update(users[i]['friends'])
    return counter
...
def friend_overlap(users):
    ...
    for i in range(len([name_user['screen_name'] for name_user in users]) - 1):
        for j in range(i+1,len([name_user['screen_name'] for name_user in users])):
            comb=set(users[i]['friends']).intersection(set(users[j]['friends']))
            friend_overlap.append(tuple([([name_user['screen_name'] for name_user in users])[i], ([name_user['screen_name'] for name_user in users])[j],len(comb)]))
    return sorted(friend_overlap, key=lambda row: (-row[2],row[0],row[1]))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    screen_names = [name_user['screen_name'] for name_user in users]
    a=0
    b=0

    for i in range(len(screen_names)):
        if screen_names[i] == 'HillaryClinton':
            a = i
        elif screen_names[i] == 'realDonaldTrump':
            b = i

    HnT_follow = set(users[a]['friends']).intersection(set(users[b]['friends']))
    resource = 'users/lookup'
    params= {'user_id': HnT_follow}
    request = robust_request(twitter,resource,params,max_tries=5)
    for HnT in request:
        return HnT['screen_name']
...
def create_graph(users, friend_counts):
    ...
    G=nx.Graph()
    H = [name_user['screen_name'] for name_user in users]
    G.add_nodes_from([name_user['screen_name'] for name_user in users])
    cnt = {a: b for a, b in friend_counts.items() if b> 1}
    for a in cnt.keys():
        for b in range(len(H)):
            if a in users[b]['friends']:
                G.add_edge(a,H[b])
    return G
...
def draw_network(graph, users, filename):
    ...
    for node in graph.nodes():
        if node in H:
            nodesnedge[node] = node
        else:
            nodesnedge[node] = ""
    options = {'node_size' : 12, 'edge_color' : '#C0C0C0' , 'node_color' : 'r' ,  'width' : 0.5}
    plt.figure(figsize=(14,14))
    nx.draw_networkx(graph, pos = None, labels=nodesnedge, **options)
    plt.axis("off")
