...
def bfs(graph, root, max_depth):
    ...
    q = deque()
    q.append(root)
    seen_nodes={a:0 for a in graph.nodes()}
    ...
    while len(q):
        ...
        node_neighbors = graph.neighbors(current_node)
        distance_present = node2distances[current_node]
        current_path = node2num_paths[current_node]

        if((distance_present < max_depth)):
            for n in node_neighbors:
                if seen_nodes[n] == 0:
                    node2distances[n] = node2distances[current_node] + 1
                    node2num_paths[n] = node2num_paths[current_node]
                    node2parents[n] = [current_node]
                    q.append(n)
                    seen_nodes[n] = 1
                else:
                    if node2distances[n] == distance_present + 1 and current_node not in node2parents[n]:
                        node2num_paths[n] += node2num_paths[current_node]
                        node2parents[n].append(current_node)
                        q.append(n)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    rough = {}
    final_dict= {}
    for n2d in node2distances.keys():
        if n2d != root:
            rough[n2d] = 1.0
    sortedn2d=sorted(node2distances.items(),key=lambda value: -value[1])
    unvisited = []
    for n in sortedn2d:
        if n[0] is not root:
            if n[0] not in unvisited:
                unvisited.append(n[0])
                for nodes_parent in node2parents[n[0]]:
                    if nodes_parent not in root:
                        rough[nodes_parent] += rough[n[0]]/len(node2parents[n[0]])
                        final_dict[tuple(sorted([n[0], nodes_parent]))] =  rough[n[0]]/len(node2parents[n[0]])
                    else:
                        final_dict[tuple(sorted([n[0], nodes_parent]))] =  rough[n[0]]/len(node2parents[n[0]])
    return final_dict
    pass
...
def approximate_betweenness(graph, max_depth):
    ...
    for n in nodes:
        ...
        resultant_vec = bottom_up(n, node2distances, node2num_paths, node2parents)
        for key,value in resultant_vec.items():
            #print(key)
            #print(resultant_vec[res])
            if key not in approxbetweeness:
                approxbetweeness[key] = value
            else:
                 approxbetweeness[key] += value

    for bet in approxbetweeness:
        approxbetweeness[bet] = (approxbetweeness[bet]/2)

    return approxbetweeness
    pass
...
def partition_girvan_newman(graph, max_depth):
    ...
    connected_comp = []
    edge_len_betweness = approximate_betweenness(graph, max_depth)
...
def get_subgraph(graph, min_degree):
    ...
    g_items = g_degree.items()
    nodes_to_remove = []
    for key,item in g_items:
        if min_degree > item:
...
def score_max_depths(graph, max_depths):
    ...
    for md in max_depths:
        final_cluster_components = partition_girvan_newman(graph, md)
        normalized_cut = norm_cut(final_cluster_components[0].nodes(), final_cluster_components[1].nodes(),graph)
        vector.append((md,normalized_cut))
...
def make_training_graph(graph, test_node, n):
    ...
    for n in sorted(graph_temp.neighbors(test_node))[:n]:
        graph_temp.remove_edge(test_node,n)
...
def path_score(graph, root, k, beta):
    ...
    node2distances, node2num_paths, node2parents =  bfs(graph, root, float("inf"))
    for n in graph_nodes:
        if n!=root and not graph.has_edge(root,n):
            pow = beta ** node2distances[n]
            num_path = node2num_paths[n]
            score_p = pow * num_path
            score_path.append(((root,n),score_p))

    sorted_val = sorted(score_path, key=lambda val: (-val[1],val[0][1]))
...
def evaluate(predicted_edges, graph):
    ...
    ###TODO
    rand = 0
    len_pe = len(predicted_edges)
    for pe_k,pe_v in predicted_edges:
        if graph.has_edge(pe_k,pe_v):
            rand += 1
    return float(rand/len_pe)
    pass
